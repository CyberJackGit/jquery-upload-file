...
this.createProgress=function(filename,answerFileId,filesize){var filepath='/';var pd=new createProgressDiv(this,s,answerFileId);pd.progressDiv.show();pd.progressbar.hide();var fileNameStr="";var fileSizeStr="";if(s.showFileCounter)
fileNameStr=obj.fileCounter+s.fileCounterStyle+filename;else fileNameStr=filename;if(s.showFileSize)
if(typeof filesize==='string'){fileSizeStr=filesize}else{fileSizeStr=getSizeStr(filesize);}
pd.fileSizeStr=fileSizeStr;pd.filename.html(fileNameStr);pd.filesize.html(fileSizeStr);if(fileNameStr.endsWith('.pdf')){pd.fileimage.addClass('file-type-pdf')}else{pd.fileimage.addClass('file-type-doc')}
...
ts.fileData=fd;var pd=new createProgressDiv(obj,s);var fileNameStr="";var fileSizeStr="";if(s.showFileCounter)fileNameStr=obj.fileCounter+s.fileCounterStyle+files[i].name
else fileNameStr=files[i].name;if(s.showFileSize)
fileSizeStr=getSizeStr(files[i].size);pd.filename.html(fileNameStr);pd.filesize.html(fileSizeStr);var form=$("<form style='display:block; position:absolute;left: 150px;' class='"+obj.formGroup+"' method='"+s.method+"' action='"+
...
{this.statusbar=$("<div class='ajax-file-upload-statusbar row' data-answer-file-id="+answerFileID+"></div>");this.fileimage=$("<div class='col-md-1' style='padding-left: 0'>"+
"<div class='ajax-file-upload-image'></div>"+
"</div>").appendTo(this.statusbar);this.maindiv=$("<div class='col-md-10 file-main-container'></div>").appendTo(this.statusbar);this.leftdiv=$("<div class='file-object-left-container'></div>").appendTo(this.maindiv);this.rightdiv=$("<div class='file-object-right-container'></div>").appendTo(this.maindiv);this.preview=$("<img class='ajax-file-upload-preview' />").width(s.previewWidth).height(s.previewHeight).appendTo(this.maindiv).hide();this.filename=$("<div class='ajax-file-upload-filename file-object-left'></div>").appendTo(this.leftdiv);this.filesize=$("<div class='ajax-file-upload-filesize file-object-left'></div>").appendTo(this.leftdiv);this.progressDiv=$("<div class='ajax-file-upload-progress'></div>").appendTo(this.maindiv).hide();this.progressbar=$("<div class='ajax-file-upload-bar'></div>").appendTo(this.progressDiv);this.progresspercentage=$("<div class='ajax-file-upload-percent'></div>").appendTo(this.rightdiv);this.abort=$("<span class='glyphicon glyphicon-remove file-remove-button'></span>").appendTo(this.rightdiv).hide();this.cancel=$("<div>"+s.cancelStr+"</div>").appendTo(this.maindiv).hide();this.done=$("<div>"+s.doneStr+"</div>").appendTo(this.maindiv).hide();this.download=$("<span class='glyphicon glyphicon-download-alt file-download-button'></span>").appendTo(this.rightdiv).hide();this.del=$("<span class='glyphicon glyphicon-remove file-remove-button'></span>").appendTo(this.rightdiv).hide();this.abort.addClass("ajax-file-upload-red");this.done.addClass("ajax-file-upload-green");this.download.addClass("ajax-file-upload-green");this.cancel.addClass("ajax-file-upload-red");this.del.addClass("ajax-file-upload-red");return this;}
function createProgressDiv(obj,s,answerFileID){var bar=null;if(s.customProgressBar)
bar=new s.customProgressBar(obj,s);else
bar=new defaultProgressBar(obj,s,answerFileID);bar.abort.addClass(obj.formGroup);bar.abort.addClass(s.abortButtonClass);bar.cancel.addClass(obj.formGroup);bar.cancel.addClass(s.cancelButtonClass);if(s.extraHTML)
...
form.remove();pd.cancel.click(function(){mainQ.splice(mainQ.indexOf(form),1);removeExistingFileName(obj,fileArray);pd.statusbar.remove();s.onCancel.call(obj,fileArray,pd);obj.selectedFiles-=fileArray.length;updateFileCounter(s,obj);});return false;},beforeSend:function(xhr,o){if(csrfToken){xhr.setRequestHeader("X-CSRFToken",csrfToken);}
...
pd.progressbar.hide();pd.abort.hide();s.onSuccess.call(this,fileArray,data,xhr,pd);if(s.showStatusAfterSuccess){if(s.showDone){pd.done.show();pd.done.click(function(){pd.statusbar.hide("slow");pd.statusbar.remove();});}else{pd.done.hide();}
